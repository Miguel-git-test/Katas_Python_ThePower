...
Donde, resumiendo, realizamos lo siguiente:
    ...
    Luego:
        jupyter nbconvert --to script Katas_Python_ThePower_v1.ipynb
...
def fmedia(lista_num,nota_aprobado=5):
    ...
    """
    if sum(lista_num)/len(lista_num)>=nota_aprobado:
        nota="aprobado"
    else:
        nota="suspenso"
    return nota
...
def tupla_a_str(tupla):
    ...
    """
    Lista_de_strings = [str(elemento) for elemento in tupla]
...
def fPromedio(lista):
    try:
        if not lista or lista==[]:
            print("La lista de números no puede estar vacía para calcular el promedio")
        else:
...
def filtro_palabras(lista_palabras,letra_inicial):
    letra_inicial_lower=letra_inicial.lower()
    return list(filter(lambda a: a[0]==letra_inicial_lower,lista_palabras))
...
def fitrar_por_nota(lista_estudiantes):
    return list(filter(lambda a: a["calificacion"]>=95, lista_estudiantes))
...
variable = "Cadena a enmascarar"
def enmascarador():
    variable_string = str(variable)
    longitud_variable = len(variable_string)
    cadena_enmascarada = "#"*(longitud_variable-4)
    cadena_visible = variable_string[-4:]
    return print(cadena_enmascarada+cadena_visible)
enmascarador()
...
# no trabaja aquí.
def buscar_puesto(nombre_completo, lista_emple ados):
    nombre_normalizado=nombre_completo.strip().lower()
    for empleado in lista_empleados:
        empleado_nombre_normalizado=empleado.get("nombre", "").strip().lower()
        if empleado_nombre_normalizado==nombre_normalizado:
            return empleado.get("puesto", "Puesto no especificado")
    return f"La persona '{nombre_completo}' no trabaja aquí."
empleados=[{"nombre": "Alberto Núñez Feijóo", "puesto": "Gerente"}]
